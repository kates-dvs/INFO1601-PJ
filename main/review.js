...
let doc = document.getElementById("previousRatings");
  console.log(data);
...
.then(json2 => {
 for(let ratings of json2.ratings){
   if(ratings.username == final){
     ...
     display(ratings.isbn, doc);
   }
}
});
}else{console.log("meh")};}

function display(isbn, doc){
...
.then(json4 => {
    for(let dets of json4.details){
      if(dets.isbn == isbn){
        ...
        doc.innerHTML += `<tr><td>${dets.bookTitle}</td></tr>`
      }
    }
});
}
